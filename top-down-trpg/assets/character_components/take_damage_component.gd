...
func _update_values():
	var inv_calc_return = inventory_component.calculate_take_damage_attributes_from_equipment()
	var inv_calc_cov = inv_calc_return[0]
	var inv_calc_mat_tier = inv_calc_return[1]
	var inv_calc_avoid =inv_calc_return[2]
	
	var att_calc_return = attributes_component.calculate_take_damage_attributes_from_attributes()
